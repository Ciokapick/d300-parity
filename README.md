...
| run ANAF's official Java validator on the same XML (oracle 3) | deferred by decision, needs a JRE + DUKIntegrator |
| rule inventory, defects found in the original | done: 17 defects, see [`docs/INVENTAR-LEGACY.md`](docs/INVENTAR-LEGACY.md) |
...
3. **Three oracles, different authorities.** The PDF's own code (what the taxpayer sees),
   the XSD (the contract), and ANAF's Java validator (the back end, deferred). Where they
...
- [`docs/INVENTAR-LEGACY.md`](docs/INVENTAR-LEGACY.md) — what the legacy actually contains, and the 17 defects found in it
